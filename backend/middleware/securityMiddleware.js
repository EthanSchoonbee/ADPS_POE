...
import {sqlInjectionSanitizer} from 'sequelize-sql-sanitizer';
...
//creating a sql injection prevention middleware method that will be used to sanitize the data from the request body
const preventSqlInjectionMiddleware = (req, res, next) => {
    try{
        req.body = sqlInjectionSanitizer(req.body);
        next();
    }catch (error){
        next(new Error('SQL Injection prevention failed.'));
    }
};
...
const securityHeadersMiddleware = helmet({
    ...
    referrerPolicy:{
        policy:'same-origin-when-cross-origin'//limits information sent in the referer header
    },
    hsts:{
